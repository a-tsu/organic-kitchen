...
try {
    ...
    $fastpay = new FastPay("fb0e5fd4cd916b369cb85043ccae0ad59ce7e931"); # シークレットを入力
    // 課金を作成(失敗した場合例外があがります)
    $charge = $fastpay->charge->create(array(
        "amount" => 1, # 金額を記載(data-amountに指定された金額と同一の必要有り）
        "card" => $token,
        "description" => "bento", # 商品の詳細を記載
    ));
//    header("Location: http://fastpay.yahoo.co.jp"); # 成功時の画面遷移先を入力
//    header("Location: http://192.168.33.10/success.php"); # 成功時の画面遷移先を入力
var_dump($charge);
